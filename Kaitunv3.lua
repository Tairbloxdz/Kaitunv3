...
-- Bắt đầu giao diện UI Delta
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/xHeptc/Kavo-UI-Library/main/source.lua"))()
local Window = Library.CreateLib("Delta Status", "DarkTheme")

-- Tạo Tab Chính
local Tab = Window:NewTab("Main")
local StatusSection = Tab:NewSection("Status")

-- Tạo nhãn (Label) thông tin
...
-- Hàm cập nhật thông tin từ Delta
local function updateAccountInfo()
    while task.wait(1) do
        -- Kiểm tra xem các thuộc tính có tồn tại không trước khi cập nhật
        local player = game.Players.LocalPlayer
        local data = player:FindFirstChild("Data") or player:FindFirstChild("Stats")
        
        -- Nếu không có data, bỏ qua và hiển thị "N/A"
        if data then
            WorldLabel:UpdateLabel("World: " .. tostring(data:FindFirstChild("World") and data.World.Value or "N/A"))
            NameLabel:UpdateLabel("Name: " .. tostring(player.Name))
            LevelLabel:UpdateLabel("Level: " .. tostring(data:FindFirstChild("Level") and data.Level.Value or "N/A"))
            FragmentsLabel:UpdateLabel("Fragments: " .. tostring(data:FindFirstChild("Fragments") and data.Fragments.Value or "0"))
            RaceLabel:UpdateLabel("Race: " .. tostring(data:FindFirstChild("Race") and data.Race.Value or "N/A"))
            EvoRaceLabel:UpdateLabel("Evo Race: " .. tostring(data:FindFirstChild("EvoRace") and data.EvoRace.Value or "N/A"))
            FruitLabel:UpdateLabel("Fruit: " .. tostring(data:FindFirstChild("Fruit") and data.Fruit.Value or "N/A"))
            FruitMasLabel:UpdateLabel("Fruit Mas: " .. tostring(data:FindFirstChild("FruitMas") and data.FruitMas.Value or "N/A"))
            AwakenLabel:UpdateLabel("Awaken: " .. (data:FindFirstChild("Awaken") and data.Awaken.Value and "✔" or "✖"))
        else
            -- Nếu không tìm thấy data, hiển thị mặc định
            WorldLabel:UpdateLabel("World: N/A")
            NameLabel:UpdateLabel("Name: " .. tostring(player.Name or "Unknown"))
            LevelLabel:UpdateLabel("Level: N/A")
            FragmentsLabel:UpdateLabel("Fragments: 0")
            RaceLabel:UpdateLabel("Race: N/A")
            EvoRaceLabel:UpdateLabel("Evo Race: N/A")
            FruitLabel:UpdateLabel("Fruit: N/A")
            FruitMasLabel:UpdateLabel("Fruit Mas: N/A")
            AwakenLabel:UpdateLabel("Awaken: ✖")
        end
    end
end

-- Chạy vòng lặp cập nhật thông tin
task.spawn(updateAccountInfo)

-- Thêm nút đóng giao diện
